...
function joinVerbs(p) { return p.length <= 1 ? p.join('') : p.length === 2 ? p[0] + ' and ' + p[1] : p.slice(0, -1).join(', ') + ', and ' + p[p.length - 1]; }
// clause join: comma even for two ("a, and b"); Oxford for 3+.
function joinClauses(p) { return p.length <= 1 ? p.join('') : p.slice(0, -1).join(', ') + ', and ' + p[p.length - 1]; }
...
export function renderMentions(actions, { emptyText = DEFAULT_EMPTY } = {}) {
  ...
  const byDid = new Map();
  for (const a of actions) { if (!byDid.has(a.did)) byDid.set(a.did, []); byDid.get(a.did).push(a); }
  const people = [...byDid.values()].map((acts) => ({ acts, maxRkey: acts.reduce((m, a) => (rkeyOf(a.recordUri) > m ? rkeyOf(a.recordUri) : m), '') }));
  people.sort((a, b) => (a.maxRkey < b.maxRkey ? 1 : a.maxRkey > b.maxRkey ? -1 : 0)); // people newest-first
  ...
  const line = ({ acts }) => {
    ...
    const byApp = new Map();
    for (const a of acts) { const k = a.appId || ''; if (!byApp.has(k)) byApp.set(k, []); byApp.get(k).push(a); }
    ...
    clauses.sort((a, b) => (a.maxRkey < b.maxRkey ? 1 : a.maxRkey > b.maxRkey ? -1 : 0)); // clauses newest-first
