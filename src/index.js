...
const DEFAULTS = {
  indexEndpoint: 'https://constellation.microcosm.blue',
  appview: 'https://public.api.bsky.app',
  userAgent: 'atmentions (+https://github.com/pixeline/atmentions)',
};
...
export async function fetchReactions(subjects, opts = {}) {
  ...
  const results = await Promise.all(
    subjectsOf(subjects).map(({ target, kind }) =>
      linksAll({
        endpoint: o.indexEndpoint,
        target,
...
function targetsFor(group, subjects) {
  ...
  return [subjects.aturi, subjects.bsky, ...urlVariants(subjects.url)].filter(
    Boolean,
  );
}
...
export async function resolveReactors(group, subjects, opts = {}) {
  ...
  const sources = group.sources || [
    { collection: group.collection, path: group.path },
  ];
  let rows = [];
  for (const { collection, path } of sources) {
    for (const target of targetsFor(group, subjects)) {
      try {
        ...
        /* skip this source/target, try the rest */
      }
    }
  }
  const seen = new Set();
  ...
  return rows.map((r) => ({
    did: r.did,
    recordUri: `at://${r.did}/${r.collection}/${r.rkey}`,
    ...(byDid.get(r.did) || { handle: r.did, displayName: '', avatar: '' }),
  }));
}
