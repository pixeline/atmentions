...
test('renderReactorList renders avatars + names, linking to the reaction record', () => {
  const html = renderReactorList([
    {
      did: 'did:plc:a',
      handle: 'alice.test',
      displayName: 'Alice',
      avatar: 'http://x/a.png',
      recordUri: 'at://did:plc:a/app.bsky.feed.like/1',
    },
  ]);
  ...
  assert.ok(
    html.includes('https://pdsls.dev/at://did:plc:a/app.bsky.feed.like/1'),
    html,
  );
});
...
test('reactorHref links a Bluesky post to the post page (not the profile)', async () => {
  ...
  assert.strictEqual(
    reactorHref({
      handle: 'pixeline.be',
      did: 'did:plc:v',
      recordUri: 'at://did:plc:v/app.bsky.feed.post/3abc',
    }),
...
test('empty reactions honor a custom empty-text (escaped)', () => {
  const html = renderHTML(
    { total: 0, groups: [] },
    { variant: 'minimal', emptyText: 'Nothing <yet>' },
  );
  assert.ok(
    html.includes('Nothing &lt;yet&gt;'),
    'custom message present and HTML-escaped',
  );
});
...
test('full variant: one row per group, app logo for known app + title, none for shared bookmark', () => {
  ...
  assert.strictEqual(
    (html.match(/data-atmo-expand=/g) || []).length,
    2,
    'one expandable row per group',
  );
  ...
  assert.ok(
    !html.includes('title="Bookmarks"'),
    'no logo/title for the shared bookmark',
  );
});

test('full variant: empty reactions still show the empty-state', () => {
  assert.ok(
    renderHTML({ total: 0, groups: [] }, { variant: 'full' }).includes(
      'atmo-empty',
    ),
  );
});
...
test('renders Lucide svg icons, not emoji, in default + full', () => {
  ...
  assert.ok(
    def.includes('<svg class="atmo-icon"'),
    'default chip uses an svg icon',
  );
  assert.ok(
    !/[✍\u{1F4DD}\u{1F516}]/u.test(def),
    'no note/bookmark emoji codepoints',
  );
  const full = renderHTML(reactions, { variant: 'full' });
  assert.ok(
    full.includes('<svg class="atmo-icon"'),
    'full row uses an svg icon',
  );
});
...
test('reactorProfileHref links to the profile (bsky for a handle, pdsls repo otherwise)', async () => {
  ...
  assert.strictEqual(
    reactorProfileHref({ handle: 'alex.test', did: 'did:plc:a' }),
    'https://bsky.app/profile/alex.test',
  );
  assert.strictEqual(
    reactorProfileHref({ handle: 'did:plc:a', did: 'did:plc:a' }),
    'https://pdsls.dev/at://did:plc:a',
  );
  assert.strictEqual(
    reactorProfileHref({ handle: '', did: 'did:plc:a' }),
    'https://pdsls.dev/at://did:plc:a',
  );
});
...
test('renderMentions composes per-person, groups by app, links verbs + name', async () => {
  ...
  const actions = [
    {
      ...
      avatar: '',
      recordUri: 'at://did:plc:a/app.bsky.feed.like/3k2',
      verb: 'liked',
      app: 'Bluesky',
      appId: 'bluesky',
    },
    {
      ...
      displayName: 'Alexandre',
      avatar: '',
      recordUri: 'at://did:plc:a/app.bsky.feed.post/3k3',
      verb: 'replied to',
      app: 'Bluesky',
      appId: 'bluesky',
    },
    {
      ...
      displayName: 'Alexandre',
      avatar: '',
      recordUri: 'at://did:plc:a/at.margin.bookmark/3k1',
      verb: 'bookmarked',
      app: 'Margin',
      appId: 'margin',
    },
  ...
  assert.ok(
    html.includes('>liked</a>') &&
      html.includes('>replied to</a>') &&
      html.includes('>bookmarked</a>'),
    'each verb is a link',
  );
  assert.ok(
    html.includes('liked</a> and <a'),
    'two verbs in a clause joined with " and " (no comma)',
  );
  assert.ok(
    html.includes('https://bsky.app/profile/alex.test'),
    'name links to profile',
  );
  assert.ok(
    html.includes('app.bsky.feed.like/3k2'),
    'verb links to its own record (like → pdsls)',
  );
});

test('renderMentions: single action collapses; no-app drops "on"; escapes', async () => {
  const { renderMentions } = await import('../src/render.js');
  const one = renderMentions(
    [
      {
        did: 'did:plc:m',
        handle: 'mondo.test',
        displayName: 'Mondo',
        avatar: '',
        recordUri: 'at://did:plc:m/app.bsky.feed.like/3kx',
        verb: 'liked',
        app: 'Bluesky',
        appId: 'bluesky',
      },
    ...
    [
      {
        ...
        displayName: 'Bob',
        avatar: '',
        recordUri: 'at://did:plc:b/community.lexicon.bookmarks.bookmark/3k9',
        verb: 'bookmarked',
        app: 'Bookmarks',
        appId: undefined,
      },
  ...
  assert.ok(
    !noApp.includes(' on Bookmarks'),
    'no-appId action drops the "on {app}" clause',
  );
  assert.ok(
    /bookmarked<\/a> this/.test(noApp),
    'reads "… bookmarked this" with no app',
  );
  const esc = renderMentions(
    [
      {
        did: 'did:plc:e',
        handle: 'x.test',
        displayName: '<b>Eve</b>',
        avatar: '',
        recordUri: 'at://did:plc:e/app.bsky.feed.like/3k',
        verb: 'liked',
        app: 'Bluesky',
        appId: 'bluesky',
      },
  ...
  assert.ok(
    esc.includes('&lt;b&gt;Eve&lt;/b&gt;') && !esc.includes('<b>Eve</b>'),
...
test('renderMentions: people ordered newest-first by rkey; empty honors emptyText', async () => {
    ...
    [
      {
        ...
        displayName: 'Older',
        avatar: '',
        recordUri: 'at://did:plc:old/app.bsky.feed.like/3aaa',
        verb: 'liked',
        app: 'Bluesky',
        appId: 'bluesky',
      },
      {
        ...
        displayName: 'Newer',
        avatar: '',
        recordUri: 'at://did:plc:new/app.bsky.feed.like/3zzz',
        verb: 'liked',
        app: 'Bluesky',
        appId: 'bluesky',
      },
  ...
  assert.ok(
    ...
    'newer rkey (3zzz) sorts first',
  );
  const empty = renderMentions([], { emptyText: 'Nothing <yet>' });
  assert.ok(
    empty.includes('atmo-empty') && empty.includes('Nothing &lt;yet&gt;'),
  );
});
